# novapilot_agents/EvaluationAgent/evaluator.py
import asyncio
import uuid
import os
...
class EvaluationAgent(AgentCommunicationInterface):
    ...
    async def _process_task(self, task: Task):
        ...
        output_message = f"Evaluation for '{task.description}' not implemented yet."
        result_data_content = {"status_message": "not_implemented", "evaluation_score": None, "evaluation_report": "No evaluation performed."}
    ...
    async def get_capabilities(self) -> List[AgentCapability]:
        return [
            AgentCapability(
                capability_id="evaluate_task_output_quality", # More specific ID
                task_type="evaluation_perform_assessment",
                description="Evaluates the output or performance of a task or another agent against specified criteria.",
                keywords=["evaluate", "assessment", "quality", "performance", "review", "check", "metrics", "score", "feedback"], # Added more keywords
                required_input_keys=["output_to_evaluate", "evaluation_criteria", "task_id_being_evaluated"], # Example keys
                generates_output_keys=["evaluation_score", "evaluation_report_summary", "feedback_items_list", "passed_evaluation"] # Example keys
            )
        ]

    async def _task_listener_loop(self):
        channel_name = "evaluation_tasks"
    ...
    async def stop_listening(self):
        ...
        task_channel_name = "evaluation_tasks"

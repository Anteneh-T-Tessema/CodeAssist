# novapilot_agents/GuardrailAgent/guardrail_checker.py
import asyncio
import uuid
import os
...
class GuardrailAgent(AgentCommunicationInterface):
    ...
    async def _process_task(self, task: Task):
        ...
        output_message = f"Guardrail check for '{task.description}' not implemented yet."
        result_data_content = {"status_message": "not_implemented", "guardrail_check_passed": None, "violations_found": []}
    ...
    async def get_capabilities(self) -> List[AgentCapability]:
        return [
            AgentCapability(
                capability_id="check_content_guardrails", # Changed from "check_guardrails" to be more specific
                task_type="guardrail_check_content",
                description="Checks provided content against defined guardrails or safety policies.",
                keywords=["guardrail", "safety", "policy", "compliance", "check", "content", "filter", "review", "validate"], # Added more keywords
                required_input_keys=["content_to_check", "policy_ids"], # policy_ids could be a list of specific policies to check against
                generates_output_keys=["guardrail_check_passed", "violations_found", "report_id", "checked_policies"]
            )
        ]

    async def _task_listener_loop(self):
        channel_name = "guardrail_tasks"
    ...
    async def stop_listening(self):
        ...
        task_channel_name = "guardrail_tasks"
